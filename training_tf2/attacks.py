...
class LowpassFilter(Layer):
    ...
    def call(self, inputs):
        ...
        signals = tf.signal.inverse_stft(
            tf.transpose(stfts, perm=[0, 2, 1]),
            self.window_length,
            self.hop_length,
            self.window_length,
        ...
        return tf.stack(
            [tf.math.real(result_stfts), tf.math.imag(result_stfts)], axis=-1
        )
...
class AdditiveNoise(Layer):
    ...
    def call(self, inputs):
        ...
        if coefficient <= self.coefficient:
            ...
            noise_signals = Add()(
                [
                    tf.random.uniform(tf.shape(signals), maxval=self.noise_strength),
                    signals,
                ]
            )
...
class CuttingSamples(Layer):
    ...
    def call(self, inputs):
        ...
        if coefficient <= self.coefficient:
            ...
            indices = tf.random.uniform_candidate_sampler(
                true_classes=tf.zeros(
                    shape=(self.batch_size, self.input_dim[0]), dtype="int64"
                ),
                num_true=self.input_dim[0],
                unique=True,
                num_sampled=self.num_samples,
                range_max=self.input_dim[0],
            ).sampled_candidates
            ...
            indices = Add()(
                [
                    ...
                    tf.repeat(
                        tf.range(
                            0,
                            self.batch_size * self.input_dim[0],
                            self.input_dim[0],
            ...
            idx = tf.scatter_nd(
                tf.reshape(indices, shape=(self.batch_size, self.num_samples, 1)),
                tf.ones((self.batch_size, self.num_samples)),
                shape=(self.batch_size * self.input_dim[0],),
            )
            ...
            signals_remove = tf.SparseTensor(
                idx_remove, values_remove, tf.shape(signals, out_type=tf.dtypes.int64)
            )
            signals_keep = tf.SparseTensor(
                idx_keep, values_keep, tf.shape(signals, out_type=tf.dtypes.int64)
            )
            output = tf.add(
                tf.sparse.to_dense(signals_remove, default_value=0.0),
                tf.sparse.to_dense(signals_keep, default_value=0.0),
            )
            output = tf.squeeze(output, axis=-1)

            result_stfts = tf.transpose(
                tf.signal.stft(
                    output, self.window_length, self.hop_length, self.window_length
                ),
                perm=[0, 2, 1],
            )
...
class ButterworthFilter(Layer):
    def __init__(
        ...
        self.conv = Conv1D(
            ...
            strides=1,
            padding="same",
            kernel_initializer=constant_initializer(self.butterworth),
            use_bias=False,
            trainable=False,
            activation=None,
        )
