...
parser.add_argument(
    "--alpha", type=float, default=0.04, help="α used at embed-time (default 0.04)"
)
...
bits_hat = (corr > 0).astype(np.int8)  # 0/1 array  (F,)

# optional: BER if you stored the ground-truth bits
truth = np.load("payload_bits.npy")  # shape (F,64) flattened later
