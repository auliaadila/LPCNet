...
class WatermarkEmbedding(Layer):
    ...
    def build(self, input_shape):
        if self.mode == "global":
            ...
            if T is None:
                T = 2400  # fallback for static-graph build
    ...
    def _broadcast_alpha(self, residual):
        ...
        if self.mode == "frame":
            ...
            return tf.clip_by_value(
                alpha_bc[:, : tf.shape(residual)[1]], 0.0, self.a_max
            )
        # mode == "cnn"
...
class WatermarkAddition(Layer):
    ...
    def _broadcast_beta(self, host):
        ...
        if self.mode == "frame":
            ...
            return tf.clip_by_value(
                beta_bc[:, : tf.shape(host)[1]], -self.b_max, self.b_max
            )
        # mode == "cnn"
        x = tf.concat(
            [host, tf.zeros_like(host)], axis=-1
        )  # dummy second channel for convs
